...
int main() {
	...
	int nh=n/2;
	ll mul1=fac(nh);
	//mul=(n*(fac(nh)%MOD))%MOD;
	ll mul=(2*((mul1%MOD)*(mul1%MOD))%MOD)%MOD;
	for(ll i=1;i<nh;i++)
		cout<<"0"<<endl;
	cout<<mul<<endl;
	mul=(n*(fac(nh)%MOD))%MOD;
	...
	for(ll i=nh+1;i<n;i++)
	{
		cout<<(mul%MOD*((num%MOD)*get_inv(den,MOD)%MOD)%MOD)%MOD<<endl;
		num=(num%MOD*num_i%MOD)%MOD;
		den=(den%MOD*den_i%MOD)%MOD;
		num_i++;
		den_i++;
